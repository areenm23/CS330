...
//printf("%lu\n",(end-start));
   	reloc_addr = mmap(NULL, end -start, PROT_READ | PROT_WRITE| PROT_EXEC,  MAP_ANONYMOUS | MAP_PRIVATE, -1, 0);	
        
      //printf("hi\n");

	 memcpy(reloc_addr, (void *)start, (end-start));	
...
int main(int argc, char **argv)
{
   ...
   if(argc){ 
	...
	//printf("hhhhh\n");
	 int main_offset = (long) &main - start_code;
        reloc_code_addr = relocate_textseg(start_code, end_data);
        reloc_main = reloc_code_addr + main_offset;
